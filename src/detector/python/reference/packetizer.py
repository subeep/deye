...
def coarse_activity(raw_data, Fs):
    # One non-overlapping 64-bin STFT is shared by legacy and modern detection.
    # 1.28--4.17 us resolution is sufficient for the 35 us burst-length tolerance.
    _, t, spectrum = signal.stft(raw_data, Fs, nfft=64, nperseg=64,
                                 noverlap=0, return_onesided=False)
    magnitude = np.abs(spectrum)
    return t, magnitude.max(axis=0) > 1.15*magnitude.mean()

def find_packet_candidate_time(raw_data, Fs, debug=False, packet_type = "droneid", legacy = False, activity=None):
    ...
    t, above_level = coarse_activity(raw_data, Fs) if activity is None else activity
    ...
    for i, _ in enumerate(peaks):
        ...
        packet_data = raw_data[max(0,int((start-start_offset)*Fs)):min(len(raw_data),int((end+end_offset)*Fs))]
