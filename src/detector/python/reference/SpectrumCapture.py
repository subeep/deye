...
class SpectrumCapture:
    ...
    def __init__(self, raw_data=None, skip_detection=False, Fs=50e6, debug=False, p_type = "droneid", legacy = False, activity=None):
    ...
    def _packetize_coarse(self):
        ...
        self.packets, cfo = find_packet_candidate_time(self.raw_data, self.sampling_rate, debug = self.debug, packet_type=self.packet_type, legacy = self.legacy, activity=self.activity)
