...
Snapshot Detector::snapshot() {
    std::lock_guard<std::mutex> lock(mutex_);
    auto out=snapshot_; out.dropped=dropped_.load(); return out;
}
...
void Detector::run() {
    ...
    try {
        ...
        const char* args[]={DETECTOR_PYTHON,"-u",DETECTOR_SCRIPT,"--turbo",DETECTOR_TURBO,nullptr};
        ...
        while (running_) {
            ...
            if (batch.data.empty() || batch.rate!=part.rate || batch.frequency!=part.frequency || batch.epoch!=part.epoch) {
                batch=std::move(part);
            } else batch.data.insert(batch.data.end(),part.data.begin(),part.data.end());
            if (batch.data.size()<static_cast<size_t>(batch.rate*.02)) continue;
            uint32_t count=static_cast<uint32_t>(batch.data.size());
            ...
            if (!transfer(sockets[0],batch.data.data(),batch.data.size()*sizeof(std::complex<float>),true,running_))
            ...
            std::lock_guard<std::mutex> lock(mutex_);
            snapshot_.samples+=batch.data.size();
            ...
            if (auto error=result.get_optional<std::string>("error")) snapshot_.status="Analysis error: "+*error;
