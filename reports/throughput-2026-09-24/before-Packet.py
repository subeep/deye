...
class Packet:
    ...
    def find_fine_start(self, samples):
        ...
        products = samples[NFFT:] * np.conj(samples[:-NFFT])
        sums = np.concatenate(([0j], np.cumsum(products, dtype=np.complex128)))
        res = sums[cpl:] - sums[:-cpl]
    ...
    def find_zc_offset(self, symbol_idx, seq, cyc):
        ...
        # Evaluate only the synchronization symbol, in a vectorized batch.
